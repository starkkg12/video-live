<script setup lang="ts">
  ...
  watch(()=> props.isActive, () => {
    if (props.isActive && !last.value ) {
      loadData(false)
      scrollContainer.value = document.querySelector('.video-popularitys-wrap .tags-scrollable .tags-content .author-wrap')
...
<template>
    <div class="author-wrap" v-if="dataList.length > 0">
        <div class="all">共{{ dataList.length }}  个创作者</div>
