...
function usePostCommentData(postId: string, size: number) {
  ...
  function transformCommentData(data: any, index: number, isChild: boolean): CommentType {
    return {
      ...
      like: data.likeCount,
      images:
        data.attachments === '' ? [] : data.attachments?.map((item: { url: string }) => utils.getImageUrl(item.url)),
      replyName: data.nickname1,
      total: isNaN(Number(data.threadCount)) ? 0 : Number(data.threadCount),
      subComments: [],
      isLiked: data.isLike === 'y',
      isDisliked: data.isDislike === 'y',
      isLoading: false,
    }
  }
  ...
  async function loadPrimaryComment() {
    try {
      ...
      const response = await service.bbs.getPrimaryComment(postId, page, size)
      if (!response.data.success) {
        console.error(response.data.errMessage)
        showFailToast(response.data.errMessage)
        return
      }
      totalCommentCount.value = response.data.data.total
  ...
  async function loadReplyComment(id: string) {
    ...
    try {
      ...
      comment.subComments.push(
        ...response.data.data.list.map((item: any, index: number) => {
          return transformCommentData(item, comment.subComments.length + index + 1, true)
        })
      )
    } catch (error) {
  ...
  async function reply(
    ...
    try {
      loading.value = true
      const hasAttachment = attachments.length ? 'y' : 'n'
      ...
      const response = await service.bbs.replyPost(
        refId,
        replyId ?? refId,
        hasAttachment,
        content,
      ...
      for (const primaryComment of commentList.value) {
        if (primaryComment.id === replyId) {
          ...
          primaryComment.subComments.unshift(
            transformCommentData(response.data.data.data, primaryComment.subComments.length + 1, false)
          )
          return true
        }
        for (const replyComment of primaryComment.subComments) {
          if (replyComment.id === replyId) {
            ...
            primaryComment.subComments.unshift(transformCommentData(response.data.data.data, 1, false))
            return true
          }
        }
      }
      ...
      commentList.value.forEach(comment => {
        comment.indexNumber !== undefined && comment.indexNumber++
      })
      commentList.value.unshift(transformCommentData(response.data.data.data, 1, true))
